...
namespace JohnStairs.RCC.Character.Motor {
    public abstract class RPGMotor : NetworkBehaviour, IRPGMotor, ITransportable {
        /// <summary>
        ...
        /// </summary>
        [Tooltip("The speed of character movement when walking is toggled off.")]
        ...
        /// <summary>
        /// The time in seconds it takes to reach the target movement speed
        /// </summary>
        [Tooltip("The time in seconds it takes to reach the target movement speed.")]
        ...
        public override void OnNetworkSpawn()
        {
            ...
            if (ClimbableLayers == 0 && climbableLayerMask != -1) {
                ClimbableLayers = 1 << LayerMask.NameToLayer("Climbable");
        ...
        protected virtual void HandleStandardMovement() {
            ...
            _movementSpeed = Mathf.SmoothDamp(_movementSpeed, _movementDirection.magnitude * maxMovementSpeed, ref _accelerationVelocity, AccelerationTime);
        ...
        protected virtual void HandleMidairMovement() {
            bool grabbedLedge = false;
            if (AllowLedgeGrabbing()) {
                grabbedLedge = TryLedgeGrab(out RaycastHit ignore);
            }
        ...
        public virtual Vector3 AlignWithCameraDirection() {
            Vector3 newFacingDirection = Utils.ProjectOnHorizontalPlane(_rpgCamera?.GetUsedCamera()?.transform.forward ?? transform.forward);
