...
namespace JohnStairs.RCC.Character.Cam {
    ...
    public class RPGCamera : NetworkBehaviour, IRPGCamera {
        /// <summary>F
        ...
        public bool OnlyIfLockedOnTarget = false;
        /// <summary>
        ...
        {
            if (IsClient && IsOwner) enabled = true;
            #region Fog handling
            ...
            if (CameraToUse == CameraSelection.MainCamera && !Camera.main)
            {
                Debug.LogWarning("Main Camera should be used but could not be found in the scene! Spawning a camera...");
                CameraToUse = CameraSelection.SpawnOwnCamera;
            }
            else if (CameraToUse == CameraSelection.AssignedCamera && !UsedCamera)
            {
            ...
            #region Camera determination
            // Determine which camera component to use
            if (CameraToUse == CameraSelection.SpawnOwnCamera)
            {
                // Create one for usage in the following code
                GameObject camObject = new GameObject(transform.name + transform.GetEntityId().GetHashCode() + " Camera");
            ...
            else if (CameraToUse == CameraSelection.MainCamera)
            {
        ...
        public override void OnNetworkSpawn()
        {
            ...
            // The player prefab is instantiated into an already-loaded scene (no
            // SceneManager.LoadScene call happens on spawn), so sceneLoaded never
            // fires for it. Run the setup immediately as well.
        ...
        protected virtual void LateUpdate() {
            ...
            if (_rpgMotor != null
                ...
                && AlignWhenMoving) {
                if (_rpgMotor.HasPlanarMovement() && !OrbitingActivated()) {
                    // Only align if the character is moving and the camera is not orbiting
        ...
        protected virtual void GetInputs(bool logWarnings = false) {
            if (!UseLegacyInputSystem) {
                // Poll inputs
                // Orbiting input
                _inputActivateOrbiting = AlwaysActivateOrbiting ? true : _inputActions.Character.ActivateOrbiting.ReadValue<float>() > 0;
                // Orbiting with character rotation input
                _inputActivateOrbitingWithCharRotation = _inputActions.Character.ActivateOrbitingwithCharacterRotation.ReadValue<float>() > 0;
                ...
                // Orbiting input
                _inputActivateOrbiting = AlwaysActivateOrbiting ? true : Utils.TryGetButton(Utils.InputPhase.Pressed, "Fire1", logWarnings);
                _inputActivateOrbitingStart = Utils.TryGetButton(Utils.InputPhase.Down, "Fire1", logWarnings);
                _inputActivateOrbitingStop = Utils.TryGetButton(Utils.InputPhase.Up, "Fire1", logWarnings);
                // Orbiting with character rotation input
                _inputActivateOrbitingWithCharRotation = Utils.TryGetButton(Utils.InputPhase.Pressed, "Fire2", logWarnings);
                _inputActivateOrbitingWithCharRotationStart = Utils.TryGetButton(Utils.InputPhase.Down, "Fire2", logWarnings);
                // Orbit rotation input
                ...
                // Zoom input
                _inputZoomAmount = Utils.TryGetAxis(Utils.InputPhase.Smoothed, "Mouse Scroll Wheel", logWarnings);
                ...
                _inputToggleMenuCursor = Utils.TryGetButton(Utils.InputPhase.Down, "Cancel", logWarnings);
            }
        }
        ...
        protected virtual void SetUpInputActionCallbacks() {
            ...
            // Orbiting input
            _inputActions.Character.ActivateOrbiting.started += context => {
                _inputActivateOrbitingStart = true;
                _cursorDragStartedOverGUI = ActivateControl && IsPointerOverGUI();
            };
            _inputActions.Character.ActivateOrbiting.canceled += context => _inputActivateOrbitingStop = true;

            // Orbiting with character rotation input
            _inputActions.Character.ActivateOrbitingwithCharacterRotation.started += context => {
                _inputActivateOrbitingWithCharRotationStart = true;
                _cursorDragStartedOverGUI = ActivateControl && IsPointerOverGUI();
            };
            _inputActions.Character.ActivateOrbitingwithCharacterRotation.canceled += context => _inputActivateOrbitingWithCharRotationStop = true;
            ...
            // Cursor-related input
            _inputActions.Character.ToggleMenuCursor.started += context => _inputToggleMenuCursor = true;
        }
        ...
        protected virtual IEnumerator ShakingCoroutine(float frequency, float amplitude, float variance) {
            ...
            while (true) {
                ...
                _shakingDelta = Random.insideUnitCircle.normalized * amplitude * Random.Range(1.0f / variance, variance);
